...
let check_typ (constr : Constr.t) : unit = 
  match Constr.kind constr with
  | Ind (inductive, _) -> ind_to_typ inductive
...
let constr_to_eq (constr : Constr.t) (ctx : context) : Z3.Expr.expr -> Z3.Expr.expr -> Z3.Expr.expr = 
  match Constr.kind constr with
...
let parse_entire (constr : Constr.t) (ctx : context) : Expr.expr =
  ...
  match constrs with
  | eq :: typ :: num1' :: num2' :: _ -> 
      let _ = check_typ typ in
      let num1 = lconstr_to_nat [num1'] 0 |> Arithmetic.Integer.mk_numeral_i ctx in
      let num2 = lconstr_to_nat [num2'] 0 |> Arithmetic.Integer.mk_numeral_i ctx in
      (constr_to_eq eq ctx) num1 num2
  | _ -> "your mom" |> failwith;;
